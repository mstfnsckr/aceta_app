...
void main() async {
  ...
  runApp(const MaterialApp(home: TamirciKayitFirma()));
}

class TamirciKayitFirma extends StatefulWidget {
  const TamirciKayitFirma({super.key});

  @override
  _TamirciKayitFirmaState createState() => _TamirciKayitFirmaState();
}

class _TamirciKayitFirmaState extends State<TamirciKayitFirma> {
  final _formKey = GlobalKey<FormState>();
  final _firmaAdiController = TextEditingController();
  final _vergiKimlikNoController = TextEditingController();
  final _yetkiliKisiController = TextEditingController();
  ...
  Future<void> _sendCode() async {
    ...
    try {
      final res = await http.post(
        Uri.parse('https://localhost:7187/api/tamirci/kodgonderfirma'),
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      final res = await http.post(
        Uri.parse('https://localhost:7187/api/tamirci/koddogrulafirma'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
  ...
  Future<void> _kayitOl() async {
    ...
    final data = {
      ...
      'vergiKimlikNo': _vergiKimlikNoController.text.trim(),
      'yetkiliKisi': _yetkiliKisiController.text.trim(),
    ...
    try {
      final res = await http.post(
        Uri.parse('https://localhost:7187/api/Tamirci/kayitfirma'),
        headers: {'Content-Type': 'application/json'},
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Tamirci Firma Kayıt', style: GoogleFonts.poppins()),
        centerTitle: true,
                ...
                child: Column(
                  children: [
                    ...
                    _buildInputField(
                      ...
                      label: 'Firma Adı',
                      icon: Icons.business,
                      validator: (v) => v!.isEmpty ? 'Firma adı girin' : null,
                    ),
                    ...
                    _buildInputField(
                      ...
                      validator: (v) {
                        if (v!.isEmpty) return 'Vergi kimlik no girin';
                    ...
                    _buildInputField(
                      ...
                      validator: (v) => v!.isEmpty ? 'Yetkili kişi adı girin' : null,
                    ),
