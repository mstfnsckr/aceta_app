import 'dart:convert';
import 'package:aceta_app/pages/2.2.0.1_TamirciBireyselGiris_Sayfasi.dart';
import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:http/http.dart' as http;

class TamirciGirisBireyselSifreYenileme extends StatefulWidget {
  const TamirciGirisBireyselSifreYenileme({super.key});

  @override
  State<TamirciGirisBireyselSifreYenileme> createState() => _SifreYenilemeState();
}

class _SifreYenilemeState extends State<TamirciGirisBireyselSifreYenileme> {
  ...
  Future<void> _sifreyiYenile() async {
    ...
    final response = await http.post(
      Uri.parse('https://localhost:7187/api/Tamirci/sifreyiYenileBireysel'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode({
        'ePosta': _ePostaController.text,
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => const TamirciGirisBireysel()),
      );
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Tamirci Bireysel Şifre Yenile', style: GoogleFonts.poppins()),
        centerTitle: true,
