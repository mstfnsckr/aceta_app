import 'dart:convert';
import 'package:aceta_app/pages/2.1.0.2_CekiciFirmaGiris_Sayfasi.dart';
import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:http/http.dart' as http;

class CekiciGirisFirmaSifreYenileme extends StatefulWidget {
  const CekiciGirisFirmaSifreYenileme({super.key});

  @override
  State<CekiciGirisFirmaSifreYenileme> createState() => _SifreYenilemeState();
}

class _SifreYenilemeState extends State<CekiciGirisFirmaSifreYenileme> {
  ...
  Future<void> _sifreyiYenile() async {
    ...
    final response = await http.post(
      Uri.parse('https://localhost:7187/api/Cekici/sifreyiYenileFirma'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode({
        'ePosta': _ePostaController.text,
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => const CekiciGirisFirma()),
      );
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Çekici Firma Şifre Yenile', style: GoogleFonts.poppins()),
        centerTitle: true,
