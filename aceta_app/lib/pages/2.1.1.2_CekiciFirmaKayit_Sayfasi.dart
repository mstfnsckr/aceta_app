...
class CekiciKayitFirma extends StatefulWidget {
  const CekiciKayitFirma({super.key});

  @override
  _CekiciKayitFirmaState createState() => _CekiciKayitFirmaState();
}

class _CekiciKayitFirmaState extends State<CekiciKayitFirma> {
  ...
  Future<void> _sendCode() async {
    ...
    try {
      final res = await http.post(
        Uri.parse('https://localhost:7187/api/cekici/kodgonderfirma'),
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      final res = await http.post(
        Uri.parse('https://localhost:7187/api/cekici/koddogrulafirma'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
  ...
  Future<void> _kayitOl() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://localhost:7187/api/Cekici/kayitfirma'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Çekici Firma Kayıt', style: GoogleFonts.poppins()),
        centerTitle: true,
                ...
                child: Column(
                  children: [
                    ...
                    _buildInputField(
                      ...
                      validator: (v) => v!.length != 10 ? '10 haneli olmalı' : null,
                    ),
                    ...
                    _buildInputField(
                      ...
                      label: 'Yetkili Kişi',
                      icon: Icons.person,
                      validator: (v) => v!.isEmpty ? 'Yetkili kişi girin' : null,
                    ),
